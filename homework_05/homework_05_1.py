...
for key, value in car_data.items():
    if value[1] >= search_criteria[0] and value[2] >= search_criteria[1] and value[4] <= search_criteria[2]:
        my_result_dict.setdefault(key, value)

sorted_price = dict(sorted(my_result_dict.items(), key=lambda item: item[1][4], reverse=True)[:5])
for key, value in sorted_price.items():
    print(f'My result: {key}: {value}')
